    ...
    if (keyutil.strongBindingObj[ch]) {
      return {
        'error': i18n.translate(
            'Key conflict: [' + ch + '] always triggers "' +
            i18n.translate(keyutil.strongBindingObj[ch]) +
            '" before your action.')
      };
      ...
      if (i != chars.length - 1) {
        return {
          'error': i18n.translate(
              'Key conflict: [' + ch + '] triggers "' +
              i18n.translate(keyutil.weakBindingObj[ch]) +
              '" before you finish typing.')
        };
      } else {
        notes.push(i18n.translate(
            '[' + ch + '] hides the default browser action "' +
            i18n.translate(keyutil.weakBindingObj[ch]) + '"'));
      }
    }
