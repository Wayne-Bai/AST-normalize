...
if (files.length !== 0 && files.indexOf("-") === - 1) stdin = falseif (doMain) main()function help()  {
   console.log(["Usage: gist [options] [filename, ...]", "Filename '-' forces gist to read from stdin.", "gist will read from stdin by default if no files specified", "    -p, --[no-]private               Make the gist private", "    -t, --type [EXTENSION]           Set syntax highlighting of the Gist by file extension", "                                     (Only applies to stdin data, filenames use extension)", "    -d, --description DESCRIPTION    Set description of the new gist", "    -o, --[no-]open                  Open gist in browser", "    -c, --[no-]copy                  Save url to clipboard (osx only)", "    -v, --version                    Print version", "    -h, --help                       Display this screen"].join("
"));
}
;
function version()  {
...
function main()  {
   ...
   if (anon) getData(files, onData.bind(null, null))    else getAuth(function(er, auth)  {
         debug("auth", er, auth);
         if (er) throw er         getData(files, onData.bind(null, auth));
      }
   )}
;
function onData(auth, er, data)  {
   if (er) throw er   var body =  {
...
;
   if (data.edit)  {
      ...
      if (prune)  {
         for (var f in data.edit.files)  {
               if (! body.files[f]) body.files[f] = null            }
      }
   }
   ...
   if (! anon) opt.headers.authorization = "token " + auth.token   debug("making request", opt);
   ...
   req.on("response", function(res)  {
         ...
         res.on("end", function()  {
               ...
               if (open) opener(url)               if (copy) copyUrl(url)               process.on("exit", function()  {
                     console.log(url);
                  }
               );
            }
...
;
   ...
   getAuthFromFile(authFile, function(er, auth)  {
         ...
         if (er) getAuthFromGit(function(er, auth)  {
               debug("getAuthFromGit", er, auth);
               if (er) getAuthFromCli(function(er, auth)  {
   ...
   function done(er, auth)  {
      if (er) return cb(er)      auth.user = auth.user.trim();
...
function getAuthFromCli(cb)  {
   ...
   var data =  {} ;
   read( {
         prompt:"github.com username: "      }, 
      function(er, user)  {
         if (er) return cb(er)         data.user = user.trim();
...
function getPassFromCli(data, cb)  {
   ...
   read( {
         prompt:"github.com password: ", 
         silent:true      }, 
      function(er, password)  {
         if (er) return cb(er)         password = password.trim();
         data.pass = password;
         tokenize(data, cb);
      }
   );
...
function tokenize(data, cb)  {
   ...
   var r =  {
      ...
      path:"/authorizations"   }
;
   if (data.otp) r.headers["X-GitHub-OTP"] = data.otp   var req = https.request(r);
   var result = "";
   req.on("response", function(res)  {
         res.on("error", cb);
         res.setEncoding("utf8");
         ...
         res.on("end", function()  {
               ...
               if (res.statusCode >= 400)  {
                  ...
                  if (res.statusCode === 401 && ! data.otp && otp && otp.match(/^required; /))  {
               ...
               if (files.length === 0) saveAuth(data, function(er)  {
                     cb(er, data);
                  }
               )                else cb(null, data)            }
         );
...
function getOTP(data, type, cb)  {
   ...
   read( {
         ...
         silent:true      }, 
      function(er, otp)  {
         if (er) return cb(er)         data.otp = otp.trim();
         tokenize(data, cb);
      }
   );
}
...
function getAuthFromFile(authFile, cb)  {
   fs.readFile(authFile, "utf8", function(er, data)  {
         if (er) return cb(er)         data = ini.parse(data);
         if (! data.gist || ! data.gist.user || ! data.gist.token) return cb(new Error("no login data in " + authFile))         return cb(null, data.gist);
      }
   );
}
;
function getAuthFromGit(cb)  {
   ...
   getConfFromGit("gist.user", function(er, user)  {
         if (er) return cb(er)         data.user = user;
         getConfFromGit("gist.token", function(er, token)  {
               if (er) return cb(er)               data.token = token;
               cb(null, data);
            }
         );
      }
...
;
   execFile("git", ["config", "--get", key], env, function(er, stdout, stderr)  {
         ...
         if (er || ! stdout) debug(stderr)         return cb(er, stdout);
      }
   );
}
;
...
function getData(files, cb)  {
   var data =  {
      files: {}    }
;
   if (stdin && files.indexOf("-") === - 1) files.push("-")   var c = files.length;
   if (edit)  {
      c++;
...
;
   ...
   function next(er)  {
      if (errState) return        else if (er) return cb(errState = er)       else if (--c === 0)  {
         cb(null, data);
      }
   }
;
...
;
   https.get(r, function(res)  {
         ...
         res.on("end", function()  {
               debug(j);
               j = JSON.parse(j);
               if (res.statusCode !== 200) cb(new Error("Invalid gist ID: " + edit))                else cb(null, j)            }
         );
      }
   ).on("error", cb);
}
