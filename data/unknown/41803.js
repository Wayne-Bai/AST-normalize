...
node;
"use strict";
var fs = require("fs"), url = require("url"), path = require("path"), request = require("request");
...
function saveURLToDep(sourceURL, filename, cb)  {
   var protocol = url.parse(sourceURL).protocol, env = process.env, proxy;
...
function download(err)  {
   var scripts = [[YUI_RUNTIME_URL, "yui-runtime.js"], ["http://cdn.sockjs.org/sockjs-0.3.min.js", "sock.js"]];
   if (err)  {
      die(err);
   }
   if (options.dev)  {
      scripts = scripts.concat([[YUI_TEST_URL, "dev/yui-test.js"], [QUNIT_JS_URL, "dev/qunit.js"], [QUNIT_CSS_URL, "dev/qunit.css"], [JASMINE_JS_URL, "dev/jasmine.js"], [JASMINE_JS_REPORTER_URL, "dev/jasmine-html.js"], [JASMINE_CSS_URL, "dev/jasmine.css"], [MOCHA_JS_URL, "dev/mocha.js"], [MOCHA_JS_ASSERTION_URL, "dev/expect.js"], [MOCHA_CSS_URL, "dev/mocha.css"], [DOJO_URL, "dev/dojo.js"], [DOJO_DOH_RUNNER_URL, "dev/dojo-doh-runner.js"]]);
   }
   scripts.forEach(function(args)  {
