...
if (/--help|-h/.test(argv[0])) return usage()function stream(alg, s)  {
   var start = Date.now();
   ...
   s.on("data", function(data)  {
         ...
         if (process.env.DEBUG) return console.log(hash.digest("hex"), Date.now() - start)         console.log(hash.digest("hex"));
      }
   );
}
;
