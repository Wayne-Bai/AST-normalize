...
cpr("./demos", "./site/demos", function(err, files)  {
      if (err) return console.log(err)      cpr("./js", "./site/js", function(err, files)  {
            if (err) return console.log(err)         }
      );
   }
);
fs.readFile("readme.md", function(err, file)  {
      if (err) return console.log(err)      var name = "index";
...
glob("docs/*.md", function(err, files)  {
      if (err) return console.log(err)      var filenames = files.map(function(name)  {
...
function writeFile(page, name)  {
   ...
   mkdirp("./site/docs", function(err)  {
         if (err) return console.error(err)         fs.writeFileSync("./site/docs/" + name + ".html", page);
      }
   );
}
;
