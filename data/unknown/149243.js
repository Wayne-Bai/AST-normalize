...
var opts = require("nomnom").script("jison-lex").option("file",  {
      flag:true, 
      position:0, 
...
exports.main = function(opts)  {
   if (opts.file)  {
      var raw = fs.readFileSync(path.normalize(opts.file), "utf8"), name = path.basename(opts.outfile || opts.file).replace(/\..*$/g, "");
...
function processGrammar(file, name)  {
   ...
   if (! settings.moduleType) settings.moduleType = opts["module-type"]   if (! settings.moduleName && name) settings.moduleName = name.replace(/-\w/g, function(match)  {
...
;
if (require.main === module) exports.main(opts.parse())