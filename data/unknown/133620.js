...
, log = console.log, dbg = debug ? console.log : emptyFn, test_utils = require("./deps/test-utils"), inspect = test_utils.inspect, format = test_utils.format, Spade = require("../"), opt =  {
      security: {
         127.0.0.1:6379: {
            db:- 1         }}    }
, client = Spade(opt), evts = [], collected = client.logger.collected, channels = [1, 2, 3], i = 0, exit = typeof done === "function" ? done : function()  {
   }
, assert = assertions || require("assert");
   ...
   client.connect(null, function()  {
         ...
         evts.push("message");
         for (; i < channels.length; ++i) evts.push("message")         client.commands.unsubscribe([], function(is_err_reply, reply, fn)  {
