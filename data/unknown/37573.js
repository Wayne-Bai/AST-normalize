...
function problemIdFromName(name)  {
   for (var id in PROBLEMS) if (PROBLEMS[id] == name) return id   throw new Error("unknown problem name: " + name);
}
;
Workshopper( {
      ...
      preMenu:function(cb)  {
         if (fs.existsSync("app.js")) return cb()         var copyCmd = process.platform == "win32" ? "copy" : "cp";
         var appVuln = path.normalize(path.join(__dirname, "..", "app-vulnerable.js"));
...
");
         ...
         console.log("When you are ready to begin the adventure, run " + this.name + " again.
");
         process.exit(1);
      }, 
      runVerifier:function(name, filename, successCb)  {
         ...
         verify(problemIdFromName(name), function(exitCode)  {
               if (! exitCode) return successCb()               process.exit(exitCode);
            }
         );
      }, 
      showHelp:function()  {
