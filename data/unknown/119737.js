...
var path = require("path"), fs = require("graceful-fs"), npmlog = require("npmlog"), temp = require("temp"), mkdirp = require("mkdirp"), rimraf = require("rimraf");
var knownOpts =  {
   config:path, 
   help:Boolean, 
...
var shortHands =  {
   ...
   v:"--level verbose"}
;
var helpString = ["", "Ares server tester.", "Usage: '" + process.argv[0] + " " + process.argv[1] + " [OPTIONS]", "", "Options:", "  -c, --config   path to ide.json        [default: '" + path.resolve(__dirname, "..", "..", "ide.json") + "]", "  -h, --help     help message            [boolean]", "  -v, --verbose  verbose execution mode  [boolean]", "  -q, --quiet    really quiet            [boolean]", ""];
var argv = require("nopt")(knownOpts, shortHands, process.argv, 2);
argv.config = argv.config || path.resolve(__dirname, "..", "..", "ide.json");
if (argv.help)  {
   helpString.forEach(function(s)  {
         console.log(s);
      }
   );
   process.exit(0);
...
describe("Testing filesystems", function()  {
      ...
      describe("fsLocal", function()  {
            var myFsPath = temp.mkdirSync( {
                  prefix:"com.palm.ares.test.fs"               }
            );
            new FsSpec( {
