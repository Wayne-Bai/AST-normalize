! ✖ / env;
node;
var debug = false, skipRemote = false;
var fs = require("fs"), child = require("child_process"), path = require("path");
var releaseVersion, nextVersion, finalFiles, isBeta, pkg, scpURL = "jqadmin@code.origin.jquery.com:/var/www/html/code.jquery.com/", cdnURL = "http://code.origin.jquery.com/", repoURL = "git@github.com:jquery/jquery-migrate.git", branch = "master", gruntCmd = process.platform === "win32" ? "grunt.cmd" : "grunt", readmeFile = "README.md", packageFile = "package.json", pluginFile = "migrate.jquery.json", devFile = "dist/jquery-migrate.js", minFile = "dist/jquery-migrate.min.js", releaseFiles =  {
   jquery-migrate-VER.js:devFile, 
   jquery-migrate-VER.min.js:minFile}
;
steps(initialize, checkGitStatus, updateReadme, tagReleaseVersion, gruntBuild, makeReleaseCopies, setNextVersion, uploadToCDN, pushToRemote, exit);
function initialize(next)  {
   ...
   var newver, oldver, rsemver = /^(\d+)\.(\d+)\.(\d+)(?:-([\dA-Za-z\-]+(?:\.[\dA-Za-z\-]+)*))?$/, version = rsemver.exec(process.argv[2] || "") || [], major = version[1], minor = version[2], patch = version[3], xbeta = version[4];
...
function updateReadme(next)  {
   ...
   if (isBeta)  {
      ...
      readme = readme.replace(/jquery-migrate-\d+\.\d+\.\d+/g, "jquery-migrate-" + releaseVersion);
      if (! debug)  {
...
function makeReleaseCopies(next)  {
   ...
   Object.keys(releaseFiles).forEach(function(key)  {
         var builtFile = releaseFiles[key], releaseFile = key.replace(/VER/g, releaseVersion);
