...
, log = console.log, dbg = debug ? console.log : emptyFn, test_utils = require("./deps/test-utils"), inspect = test_utils.inspect, format = test_utils.format, Spade = require("../"), opt =  {
      ...
      security: {
         127.0.0.1:6380: {
            requirepass:"secret"         }} , 
      queue: {
         timestamps:true      }} , client = Spade(opt), Vapid = null, vp = null, vapid_opt =  {
      secret:"secret", 
      maxdb:16   }
, vport = 6380, evts = [], collected = client.logger.collected, intval = 2000, exit = typeof done === "function" ? done : function()  {
   }
, assert = assertions || require("assert");
   ...
   client.cli(true, function(ename, args)  {
      ...
      true);
   log("- now #initTasks.");
   client.initTasks(["connection", "notexistentfile"]);
   ...
   client.connect(null, function()  {
         ...
         client.on("polling", function()  {
               if (vp.silent) return                log("- now #mute Vapid Server.. ");
               vp.mute();
            }
         );
