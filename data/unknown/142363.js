...
var server = connect().use(function(req, res, next)  {
      if (options.index !== "index.html" && url.parse(req.url).pathname.match(/^(\/|\/index.html)$/))  {
         send(req, "/" + options.index,  {
               root:options.root            }
         ).pipe(res);
      }
       else  {
...
).use(serveStatic(options.root)).use(function(req, res, next)  {
...
http.createServer(server).listen(options.port).once("error", function(err)  {
