...
var main = module.exports = function(versions, tests, restore)  {
   ...
   Object.keys(versions).forEach(function(version)  {
         ...
         if (headers && headers.length) headers = headers.map(function(header)  {
         ...
         function createComment(paramsStruct, section, funcName, indent)  {
            ...
            var comment = [indent + "/** section: github", indent + " *  " + section + "#" + funcName + "(msg, callback) -> null", indent + " *      - msg (Object): Object that contains the parameters and their values to be sent to the server.", indent + " *      - callback (Function): function to call when the request is finished " + "with an error as first argument and result data as second argument.", indent + " *", indent + " *  ##### Params on the `msg` object:", indent + " *"];
            comment.push(indent + " *  - headers (Object): Optional. Key/ value pair " + "of request headers to pass along with the HTTP request. Valid headers are: " + "'" + defines["request-headers"].join("', '") + "'.");
            if (! params.length) comment.push(indent + " *  No other params, simply pass an empty Object literal `{}`")            var paramName, def, line;
            for (var i = 0, l = params.length; i < l; ++i)  {
                  paramName = params[i];
                  if (paramName.charAt(0) == "$")  {
                     paramName = paramName.substr(1);
                     if (! defines.params[paramName])  {
                        Util.log("Invalid variable parameter name substitution; param '" + paramName + "' not found in defines block", "fatal");
                        process.exit(1);
                     }
                      else def = defines.params[paramName]                  }
                   else def = paramsStruct[paramName]                  line = indent + " *  - " + paramName + " (" + def.type || "mixed" + "): " + def.required ? "Required. " : "Optional. ";
                  if (def.description) line = def.description                  if (def.validation) line = " Validation rule: ` " + def.validation + " `."                  comment.push(line);
               }
            return comment.join("
") + "
" + indent + " **/";
...
;
         function getParams(paramsStruct, indent)  {
            var params = Object.keys(paramsStruct);
            if (! params.length) return "{}"            var values = [];
            var paramName, def;
            for (var i = 0, l = params.length; i < l; ++i)  {
                  paramName = params[i];
                  if (paramName.charAt(0) == "$")  {
                   ...
                   else def = paramsStruct[paramName]                  values.push(indent + "    " + paramName + ": "" + def.type + """);
               }
            return "{
" + values.join(",
") + "
...
;
         function prepareApi(struct, baseType)  {
            if (! baseType) baseType = ""            Object.keys(struct).forEach(function(routePart)  {
                  var block = struct[routePart];
                  if (! block) return                   var messageType = baseType + "/" + routePart;
                  if (block.url && block.params)  {
                     var parts = messageType.split("/");
                     var section = Util.toCamelCase(parts[1].toLowerCase());
                     if (! block.method)  {
                        throw new Error("No HTTP method specified for " + messageType + "in section " + section);
                     }
                     parts.splice(0, 2);
                     var funcName = Util.toCamelCase(parts.join("-"));
                     var comment = createComment(block.params, section, funcName, "    ");
                     if (! sections[section]) sections[section] = []                     var afterRequest = "";
                     if (headers && headers.length)  {
                        afterRequest = AfterRequestTpl.replace("<%headers%>", """ + headers.join("", "") + """);
                     }
                     sections[section].push(HandlerTpl.replace("<%funcName%>", funcName).replace("<%comment%>", comment).replace("<%afterRequest%>", afterRequest));
                     if (! testSections[section]) testSections[section] = []                     testSections[section].push(TestHandlerTpl.replace("<%name%>", block.method + " " + block.url + " (" + funcName + ")").replace("<%funcName%>", section + "." + funcName).replace("<%params%>", getParams(block.params, "            ")));
                  }
                   else  {
...
;
         ...
         Fs.writeFileSync(Path.join(dir, "index.js"), IndexTpl.replace("<%name%>", defines.constants.name).replace("<%description%>", defines.constants.description).replace("<%scripts%>", """ + sectionNames.join("", "") + """), "utf8");
         Object.keys(sections).forEach(function(section)  {
               var def = sections[section];
               Util.log("Writing '" + section + ".js' file for version " + version);
               Fs.writeFileSync(Path.join(dir, section + ".js"), SectionTpl.replace(/<%sectionName%>/g, section).replace("<%sectionBody%>", def.join("
")), "utf8");
               if (! tests) return                def = testSections[section];
               var body = TestSectionTpl.replace("<%version%>", version.replace("v", "")).replace(/<%sectionName%>/g, section).replace("<%testBody%>", def.join("

"));
               var path = Path.join(dir, section + "Test.js");
               if (Fs.existsSync(path) && Math.abs(Fs.readFileSync(path, "utf8").length - body.length) >= 20)  {
                  Util.log("Moving old test file to '" + path + ".bak' to preserve tests " + "that were already implemented. 
Please be sure te check this file " + "and move all implemented tests back into the newly generated test!", "error");
...
Usage: $0 [-r] [-v VERSION]").alias("r", "restore").describe("r", "Restore .bak files, generated by a previous run, to the original").alias("v", "version").describe("v", "Semantic version number of the API to generate. Example: '3.0.0'").alias("t", "tests").describe("t", "Also generate unit test scaffolds").alias("h", "help").describe("h", "Display this usage information").boolean(["r", "t", "h"]).argv;
   ...
   Fs.readdirSync(baseDir).forEach(function(version)  {
         ...
         if (! Fs.existsSync(path)) return          var routes;
         try {
         ...
         if (! routes.defines) return          availVersions[version] = routes;
      }
   );
   ...
   var versions =  {} ;
   if (argv.version)  {
      if (argv.version.charAt(0) != "v") argv.version = argv.v = "v" + argv.version      if (! availVersions[argv.version])  {
