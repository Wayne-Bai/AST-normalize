...
if (args.length > 0)  {
   ...
   if (args[0] === "create")  {
      if (args.length === 1)  {
         ...
         "", "Example apps: hello-world-angular hello-world-jquery";
         ✖.join("
");
         ;
      }
...
");
      ...
      if (fs.existsSync(angusLocation))  {
         require("./core/gulpfile.js")(argv, cwd);
      }
       else  {
   ...
   console.log(["usage: angus <command> [<args]", "", "create     Creates an app folder with basic structure", "dev        Runs Angus in development mode (must be inside app directory)", "prod       Runs Angus in production mode (must be inside app directory)", "sound      Configure Angus to use sounds on build success/failure", "", "Any other command will be parsed as a gulp task. Please see the angus/core/gulp/ folder for available tasks."].join("
"));
}
