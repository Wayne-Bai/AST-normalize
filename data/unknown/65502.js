! ✖ / env;
node;
var buffer = "", path = require("path"), fs = require("fs"), mu = require("mu"), counter = 0, version = fs.readFileSync("VERSION").toString();
function presenter(doc)  {
   if (doc.ignore || doc.isPrivate) return    var memberOf = doc.tags.filter(function(doc)  {
...
;
   ...
   return  {
      ...
      code:doc.code, 
      params:doc.tags.filter(function(tag)  {
            return tag.type === "param";
         }
      ), 
      has_params:! ! doc.tags.filter(function(tag)  {
            return tag.type === "param";
         }
      ).length, 
      tags:doc.tags, 
      module:doc.tags.some(function(tag)  {
            return tag.type === "module" || tag.type === "constructor";
         }
      ), 
      parent:parent(), 
      related:related, 
...
process.stdin.on("end", function()  {
      ...
      mu.compile("./docs/template.mustache", function(err, parsed)  {
            if (err) throw err            mu.render(parsed,  {
                  modules:modules, 
