...
function MtGoxStream(opts)  {
   ...
   this._read = function()  {
      if (ws) return       var url = "wss://websocket.mtgox.com";
      ws = new Websocket(url);
      ws.on("open", function()  {
            console.log("connected to:", url);
            if (opts.ticker) subscribe("ticker.BTC" + opts.currency)            if (opts.depth) subscribe("depth.BTC" + opts.currency)            if (opts.trade) subscribe("trade.BTC")            if (opts.lag) subscribe("trade.lag")         }
      );
...
function currencies()  {
   return ["USD", "AUD", "CAD", "CHF", "CNY", "DKK", "EUR", "GBP", "HKD", "JPY", "NZD", "PLN", "RUB", "SEK", "SGD", "THB"];
}
;
if (! module.parent)  {
   var usd = new MtGoxStream();
   usd.pipe(process.stdout);
   var eur = new MtGoxStream( {
         currency:"EUR", 
         ticker:false, 
         depth:true      }
   );
