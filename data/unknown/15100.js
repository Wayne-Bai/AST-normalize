...
require("colors");
console.log([]);
"             \".green + "||" + "/".green, "             \".green + "||" + "/".green, "           .<><><>.".green, "          .<><><><>.".green, "          '<><><><>'".green, "pineapple".cyan + "  '<><><>'".green, ✖;
✖.join("
");
;
var opts, cmds, cmd, args, match, bin, binCommands, binNamespace, call, preservedArgs, exex, pwd, i, func, binOpts;
...
global.parser = new parseopts.Parser(opts);
try {
   parser.parse(args);
   ...
   pineapple.on("extensions:ready", function()  {
         opts.extensions.map(function(extension)  {
               if (pineapple[extension] instanceof Object && pineapple[extension].init instanceof Function)  {
                  ...
                  var config = pineapple.config.extensions && typeof pineapple.config.extensions[extension] === "object" ? pineapple.config.extensions[extension] :  {} ;
...
if (typeof bin[cmd] === "object" && typeof call = bin[cmd].call === "function")  {
   if (bin[cmd].deps && bin[cmd].deps.length)  {
      for (i = 0; i < bin[cmd].deps.length; i++)  {
            if (typeof bin[bin[cmd].deps[i]] === "object" && typeof func = bin[bin[cmd].deps[i]].call === "function")  {
