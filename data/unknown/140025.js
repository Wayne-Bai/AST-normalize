...
function main(args)  {
   ...
   args = args.map(function(x)  {
         if (x[0] == "-" && x[1] != "-") return "-" + x         return x;
      }
   ).filter(Boolean);
   if (args[2] && args[2][0] != "-" || args[2].indexOf("h") != - 1) type = args[2]   var i = args.indexOf("--target");
   if (i != - 1 && args[i + 1]) BUILD_DIR = args[i + 1]   if (args.indexOf("--h") == - 1)  {
      if (type == "minimal")  {
         buildAce( {
...
function showHelp(type)  {
   ...
   console.log("");
   if (type) console.log("  output for " + type + " generated in " + BUILD_DIR)}
;
function ace()  {
...
function demo()  {
   ...
   function changeComments(data)  {
      return data.replace("doc/site/images/ace-logo.png", "demo/kitchen-sink/ace-logo.png").replace(/<!\-\-DEVEL[\d\D]*?DEVEL\-\->/g, "").replace(/PACKAGE\-\->|<!\-\-PACKAGE/g, "").replace(/\/\*DEVEL[\d\D]*?DEVEL\*\//g, "").replace(/PACKAGE\*\/|\/\*PACKAGE/g, "").replace("%version%", version).replace("%commit%", ref);
   }
;
   ...
   copy(ACE_HOME + "/demo/", BUILD_DIR + "/demo/",  {
         ...
         replace:function(source)  {
            if (! /^\s*</.test(source)) return source            var removeRequireJS;
...
;
                  function script(str)  {
                     result.push("<script src="../src/" + str + ".js"></script>");
                  }
;
...
");
            ...
            if (removeRequireJS) source = source.replace(/\s*\}\);?\s*(<\/script>)/, "
$1")            source = source.replace(/"\.\.\/build\//g, function(e)  {
                  console.log(e);
                  return ""../";
               }
            );
...
function jsFileList(path, filter)  {
   path = ACE_HOME + "/" + path;
   if (! filter) filter = /_test/   return fs.readdirSync(path).map(function(x)  {
         if (x.slice(- 3) == ".js" && ! filter.test(x) && ! /\s|BASE|(\b|_)dummy(\b|_)/.test(x)) return x.slice(0, - 3)      }
   ).filter(Boolean);
}
;
...
function buildAceModule(opts, callback)  {
   if (! buildAceModule.queue)  {
      ...
      buildAceModule.dequeue = function()  {
         if (buildAceModule.running) return          var call = buildAceModule.queue.shift();
         buildAceModule.running = call;
         if (call) buildAceModuleInternal.apply(null, call)      }
;
   }
...
function buildAceModuleInternal(opts, callback)  {
   ...
   if (cache && cache.configs && cache.configs[key]) return write(null, cache.configs[key])   var pathConfig =  {
      paths: {
...
;
      ...
      if (! opts.outputFile) return callback(err, result)      var code = result.code;
      if (opts.compress)  {
         if (! result.codeMin) result.codeMin = compress(result.code)         code = result.codeMin;
      }
      var targetDir = BUILD_DIR + "/src";
      if (opts.compress) targetDir = "-min"      if (opts.noconflict) targetDir = "-noconflict"      var to = /^([\\/]|\w:)/.test(opts.outputFile) ? opts.outputFile : path.join(opts.outputFolder || targetDir, opts.outputFile);
      var filters = [];
      var ns = opts.ns || "ace";
      if (opts.filters) filters = filters.concat(opts.filters)      if (opts.noconflict) filters.push(namespace(ns))      var projectType = opts.projectType;
      if (projectType == "main" || projectType == "ext")  {
         filters.push(exportAce(ns, opts.require[0], opts.noconflict ? ns : "", projectType == "ext"));
      }
...
function buildAce(options)  {
   ...
   modeNames.forEach(function(name)  {
         if (snippetFiles.indexOf(name + ".js") == - 1) addSnippetFile(name)         buildSubmodule(options,  {
...
function namespace(ns)  {
   return function(text)  {
      text = text.toString().replace(/ACE_NAMESPACE\s*=\s*""/, "ACE_NAMESPACE = "" + ns + """).replace(/(\.define)|\bdefine\(/g, function(_, a)  {
            return a || ns + ".define(";
         }
      );
      return text;
      ...
      var template = function()  {
         function()  {
            REQUIRE_NS.require(MODULES, function(a)  {
                  ...
                  if (! window.NS) window.NS = a                  for (var key in a) if (a.hasOwnProperty(key)) window.NS[key] = a[key]               }
            );
         }
();
      }
...
");
      if (typeof modules == "string") modules = [modules]      return text.replace(/;\s*$/, "") + ";" + template.toString().replace(/MODULES/g, JSON.stringify(modules)).replace(/REQUIRE_NS/g, requireBase).replace(/NS/g, ns).slice(13, - 1);
   }
;
}
;
function updateModes()  {
   modeList().forEach(function(m)  {
         ...
         if (! /this.\$id\s*=\s*"/.test(source)) source = source.replace(/\n([ \t]*)(\}\).call\(\w*Mode.prototype\))/, "
$1    this.$id = "";
$1$2")         source = source.replace(/(this.\$id\s*=\s*)"[^"]*"/, "$1"ace/mode/" + m + """);
...
function generateThemesModule(themes)  {
   var themelist = ["define(function(require, exports, module) {", "
...
function compress(text)  {
   ...
   ujs.options.mangle_toplevel =  {
      except:["ACE_NAMESPACE", "requirejs"]   }
;
   ujs.options.beautify =  {
      ascii_only:true, 
      inline_script:true   }
;
   return ujs(text);
