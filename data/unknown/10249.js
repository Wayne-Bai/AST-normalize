...
var argv = require("optimist").boolean("r").default("r", false).argv;
...
function checkFile(fileName)  {
   fs.readFile(path.resolve(fileName), "utf8", function(err, str)  {
         if (err) throw err         var err = leaky(str, fileName);
         if (err)  {
            console.log(err);
            process.exit(1);
         }
...
function getFiles(err, files)  {
   if (err) throw err   files.forEach(checkFile);
}
;
fs.stat(path.resolve(input), function(err, stats)  {
      if (err) throw err      if (stats.isDirectory())  {
