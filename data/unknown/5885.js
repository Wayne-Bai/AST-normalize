...
function highlight(text, language)  {
   if (language === false) return []   var highlighted;
   if (language)  {
      try {
   ...
   if (! highlighted) highlighted = hljs.highlightAuto(text)   var $ = cheerio.load(highlighted.value);
   var ranges = [];
   do  {
         ...
         var elCount = $("*:not(:has(*))").replaceWith(function()  {
               ...
               [this].concat($el.parents().get(), [$.root()]).reverse().reduce(function(parent, el)  {
                     $(parent).contents().each(function()  {
                           var $sibling = $(this);
                           if ($sibling.is(el)) return false                           text = $sibling.text();
                        }
                     );
                     return el;
                  }
 ...
 while (lastElCount !== elCount)   return ranges;
}
;
;
