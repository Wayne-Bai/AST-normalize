...
program.option("-p, --port <n>", "Server port", parseInt).option("-l, --livereload [n]", "Livereload port").parse(process.argv);
process.env.PORT = program.port || process.env.PORT || 3000;
if (program.livereload)  {
   process.env.LIVERELOAD = true === program.livereload ? process.env.LIVERELOAD || 35729 : program.livereload;
}
...
;
                     if (process.env.LIVERELOAD)  {
                        config.livereload = "//" + req.hostname + ":" + process.env.LIVERELOAD + "/livereload.js";
                     }
                      else  {
