...
program.command("start").usage("[Options] [Processes] e.g. web=1,log=2,api").option("-s, --showenvs", "show ENV variables on start", false).option("-x, --proxy     <PORT>", "start a load balancing proxy on PORT").option("-f, --forward   <PORT>", "start a forward proxy on PORT").option("-i, --intercept <HOSTNAME>", "set forward proxy to intercept HOSTNAME", null).option("-t, --trim      <N>", "trim logs to N characters", 0).option("-w, --wrap", "wrap logs (negates trim)").description("Start the jobs in the Procfile").action(function(command_left, command_right)  {
...
program.command("run").usage("[Options]").option("-s, --showenvs", "show ENV variables on start", false).description("Run a one off process using the ENV variables").action(function(command_left, command_right)  {
      command = command_right || command_left;
      var envs = loadEnvs(program.env);
...
program.command("export").option("-a, --app  <NAME>", "export upstart application as NAME", "foreman").option("-u, --user <NAME>", "export upstart user as NAME", "root").option("-o, --out  <DIR>", "export upstart files to DIR", ".").option("-c, --cwd  <DIR>", "change current working directory to DIR").option("-g, --gid  <GID>", "set gid of upstart config to GID").option("-l, --log  <DIR>", "specify upstart log directory", "/var/log").option("-t, --type <TYPE>", "export file to TYPE (default upstart)", "upstart").option("-m, --template <DIR>", "use template folder").description("Export to an upstart job independent of foreman").action(function(command_left, command_right)  {
      command = command_right || command_left;
      var envs = loadEnvs(program.env);
