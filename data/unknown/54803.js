...
if (argv.h) usage(0) else if (argv._[0] === "search")  {
   if (! argv._[1]) return error("Please add a keyword for search.")   var keyword = argv._[1];
   api.search(keyword, function(err, res)  {
         if (err) return error(err)         var data = res.result;
   ...
   if (! argv._[1]) return error("Please add a album ID.")   var id = argv._[1];
   api.album(id, function(err, res)  {
         if (err) return error(err)         console.log("Album "" + res.album.name + "" by "" + res.album.artist.name + "" list: ");
   ...
   if (! argv._[1]) return error("Please add a playlist ID.")   var id = argv._[1];
   api.playlist(id, function(err, res)  {
         if (err) return error(err)         console.log("Playlist by "" + res.result.creator.nickname + "" List: ");
   ...
   if (! argv._[1]) return error("Please add a dj ID.")   var id = argv._[1];
   api.dj(id, function(err, res)  {
         if (err) return error(err)         console.log("DJ by "" + res.program.dj.nickname + "" List: ");
   ...
   if (! argv._[1]) return error("Please add a song ID.")   var id = argv._[1];
   api.detail(id, function(err, res)  {
         if (err) return error(err)         if (res.songs)  {
   ...
   if (argv._.length < 2) return error("please set the type you want to play by "-t TYPE "")   var id = argv._[1];
   var type = argv.type || argv.t;
   getList(id, type, function(err, songs)  {
         if (err) return error(err)         playList(songs, function()  {
   ...
   getList(id, type, function(err, songs, dir)  {
         if (err) return error(err)         download(songs, dist, dir);
      }
   );
}
 else usage(1)function usage(code)  {
...
function getList(id, type, cb)  {
   ...
   if (type === "album")  {
      api.album(id, function(err, res)  {
            if (err) cb(err)            var name = res.album.name;
            res.album.songs.forEach(function(song)  {
                  songs.push( {
                        name:song.name, 
      ...
      api.playlist(id, function(err, res)  {
            if (err) cb(err)            var name = res.result.name;
            res.result.tracks.forEach(function(song)  {
                  songs.push( {
                        name:song.name, 
                        artist:song.artists[0].name, 
      ...
      api.dj(id, function(err, res)  {
            if (err) cb(err)            var name = res.program.name;
            res.program.songs.forEach(function(song)  {
                  songs.push( {
                        name:song.name, 
                        artist:song.artists[0].name, 
      ...
      api.detail(id, function(err, res)  {
            if (err) cb(err)            res.songs.forEach(function(song)  {
                  songs.push( {
                        name:song.name, 
                        artist:song.artists[0].name, 
                        src:song.mp3Url                     }
...
function playList(songs, cb)  {
   eachSeries(songs, play, function(err)  {
         if (err) return error(err)         cb(null);
      }
   );
}
;
function play(song, cb)  {
   ...
   var request = hyperquest.get(song.src).pipe(new lame.Decoder()).pipe(new Speaker()).on("finish", function()  {
...
function download(songs, dist, dir)  {
   ...
   mkdirp(dirName, function(err)  {
         if (err) return error(err)         console.log(chalk.blue("Created folder: ", dirName));
...
;
function error(err)  {
   if (! err) return    console.error(chalk.red(String(err)));
   process.exit(1);
}
;
