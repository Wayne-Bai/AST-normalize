! ✖ / env;
node;
var couchapp = require("./main.js"), watch = require("watch"), path = require("path"), fs = require("fs");
function abspath(pathname)  {
   if (pathname[0] === "/") return pathname   return path.join(process.cwd(), path.normalize(pathname));
}
;
         ...
         for (i in files)  {
               function(i)  {
                  if (files[i].isDirectory())  {
                     try {
                        fs.mkdirSync(i.replace(source, dest), 493);
                     }
                     catch (e) {
...
function isUsingDirectoryConfig()  {
   if (_isUsingDirectoryConfig != null) return _isUsingDirectoryConfig   return _isUsingDirectoryConfig = process.argv[2] && process.argv[2].trim() === "-dc";
}
;
if (process.mainModule && process.mainModule.filename === __filename)  {
   var node, bin, command, app, couch, configDirectory, configFileNames, apps = [], beforePushSyncListener, afterPushSyncListener;
   ...
   if (command == "help" || command == undefined)  {
      console.log(["couchapp -- utility for creating couchapps", "", "Usage:", "(backwardly compatible without switch - single app file)", "  couchapp <command> app.js http://localhost:5984/dbname [opts]", "(directory based config specified by switch - multiple app files and pre- and post-processing capability)", " couchapp -dc <command> <appconfigdirectory> http://localhost:5984/dbname", "", "Commands:", "  push   : Push app once to server.", "  sync   : Push app then watch local files for changes.", "  boiler : Create a boiler project.", "  serve  : Serve couchapp from development webserver", "            you can specify some options ", "            -p port  : list on port portNum [default=3000]", "            -d dir   : attachments directory [default='attachments']", "            -l       : log rewrites to couchdb [default='false']"].join("
"));
      process.exit();
   }
   if (couch == undefined)  {
...
(i);
      ...
      if (command == "boiler")  {
         ...
         couchapp.createApp(require(abspath(app)), couch, function(app)  {
               if (command == "push") app.push()                else if (command == "sync") app.sync()                else if (command == "serve") serve(app)            }
         );
      }
   }
}
function serve(app)  {
   var url = require("url");
   var port = 3000, staticDir = "attachments", tmpDir = "/tmp/couchapp-compile-" + process.pid, logDbRewrites = false;
   var arg;
   ...
   for (var i in app.doc.rewrites)  {
         ...
         if (rw.to.indexOf(dbPrefix) == 0)  {
            ...
            if (dbPath.indexOf("*") >= 0)  {
               ...
               proxyPaths[proxyPath] = dbUrlObj.protocol + "//" + dbUrlObj.host + path.normalize(dbUrlObj.pathname + dbPath);
               if (logDbRewrites)  {
   ...
   var httpProxy = require("http-proxy"), connect = require("connect"), logger = require("morgan"), serveStatic = require("serve-static"), errorHandler = require("errorhandler"), connectCompiler;
   try {
   ...
   var app = connect().use(logger("dev")).use(serveStatic(staticDir));
   if (connectCompiler)  {
