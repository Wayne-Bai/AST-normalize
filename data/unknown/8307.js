! ✖ / env;
node;
var cp = require("child_process"), fs = require("fs"), path = require("path"), Mocha = require("mocha");
var force = false, debug = false;
var arch = process.arch, platform = process.platform, v8 = /[0-9]+\.[0-9]+/.exec(process.versions.v8)[0];
...
function build()  {
   cp.spawn(process.platform === "win32" ? "node-gyp.cmd" : "node-gyp", ["rebuild"].concat(args),  {
         ...
         if (err)  {
            if (err === 127)  {
               console.error("node-gyp not found! Please upgrade your install of npm! You need at least 1.1.5 (I think) " + "and preferably 1.1.30.");
            }
             else  {
