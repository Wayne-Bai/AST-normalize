...
var commands =  {
   ...
   replicate:null, 
   help:[ {
      list:["clear-cache", "create", "createdb", "deletedb", "listdb", "replicate", "help", "install", "update", "ls", "pack", "publish", "push", "show", "transform", "unpublish", "upload", "uuids"]   }
], 
   install:[ {
      directories:true, 
      filenames:/.*\.tar\.gz$/   }
...
;
function matchList(list, curr, nextlist)  {
   var m = [];
...
function completeList(argdef)  {
   ...
   if (argdef.directories)  {
      l = l.concat(fs.readdirSync(".").filter(function(f)  {
...
if (arglen === 0)  {
   ...
   matches = matchList(Object.keys(commands), curr, commands[curr] && completeList(commands[curr][0]));
}
 else if (arglen > 1)  {
   if (commands[command] && commands[command][arglen - 2])  {
      ...
      if (argdef.list)  {
         matches = matches.concat(matchList(argdef.list, curr, completeList(next_argdef)));
      }
      if (argdef.directories)  {
         var wd = "./";
         if (curr && /\/$/.test(curr))  {
         ...
         matches = matches.concat(matchList(dirs, curr, completeList(next_argdef)));
      }
      if (argdef.filenames)  {
         var wd = "./";
         if (curr && /\/$/.test(curr))  {
