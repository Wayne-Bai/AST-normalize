...
var assert = require("assert"), fs = require("fs"), Address = require("../../app/models/Address"), TransactionDb = require("../../lib/TransactionDb").default(), addrValid = JSON.parse(fs.readFileSync("test/integration/addr.json")), utxoValid = JSON.parse(fs.readFileSync("test/integration/utxo.json"));
            ...
            if (v.disabled)  {
               ...
               it("Address info (cache) for: " + v.addr, function(done)  {
                     ...
                     a.update(function(err)  {
                           ...
                           if (v.txApperances) a.txApperances.should.equal(v.txApperances, "txApperances")                           if (v.totalReceived) a.totalReceived.should.equal(v.totalReceived, "totalReceived")                           if (v.totalSent) assert.equal(v.totalSent, a.totalSent, "send: " + a.totalSent)                           if (v.balance) assert.equal(v.balance, a.balance, "balance: " + a.balance)                           done();
                        }, 
                         {
                           txLimit:0                        }
                     );
      ...
      utxoValid.forEach(function(v)  {
            if (v.disabled)  {
               ...
               it("Address unspent for: " + v.addr, function(done)  {
                     ...
                     var a = new Address(v.addr, txDb);
                     a.update(function(err)  {
                           if (err) done(err)                           assert.equal(v.addr, a.addrStr);
                           if (v.length) a.unspent.length.should.equal(v.length, "Unspent count")                           if (v.tx0id)  {
