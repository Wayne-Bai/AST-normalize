...
if (cmd === "build") build( {
      watch:false   }
) else if (cmd === "watch") build( {
      watch:true   }
) else usage(1)function build(opts)  {
   var js = opts.watch ? watchify : browserify;
   js("./browser/main.js").transform(workerify).bundle().pipe(fs.createWriteStream("./static/bundle.js"));
   var assets = vinyl.src("./browser/assets/**/*");
   assets.pipe(vinyl.dest("./static/assets"));
   var index = catw("./browser/index.html",  {
         watch:opts.watch      }
   );
