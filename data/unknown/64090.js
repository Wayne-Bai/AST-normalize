...
module.exports = function(grunt)  {
   "use strict";
   var prevVersion, major, minor, patch, baseVersion, parts, repo, baseDir, repoDir, linkNextPage, changelog, changelogPath, done, issues = [], firstRun = true, pkg =  {
      version:"1.4.0pre"   }
, rnewline = /\r?\n/, fs = require("fs"), $ = require("jquery");
   grunt.registerTask("changelog:create", function()  {
         done = this.async();
         walk([bootstrap, getVersions, generateChangelog]);
      }
   );
   function setup(fn)  {
...
;
   function parseHttpHeaders(jqXHR)  {
      var linksArray, linksJSON, headersArray = jqXHR.getAllResponseHeaders().split("
"), headersJSON =  {} ;
      $.each(headersArray, function(index, value)  {
            var delIdx = value.indexOf(":"), key = value.substr(0, delIdx);
...
;
   function recurse()  {
      if (linkNextPage != null)  {
         $.when(getIssues(linkNextPage)).then(function()  {
               recurse();
            }
         ).fail(function()  {
               abort("Failed getting next page...");
            }
         );
      }
       else  {
...
;
   function generateChangelog()  {
      var commits, fullFormat = "* %s (TICKETREF, [%h](http://github.com/jquery/jquery-mobile/commit/%H))";
...
", changelog = changelog.replace("{title}", "jQuery Mobile " + baseVersion + " Changelog");
      ...
      changelog = commits.map(function(commit)  {
            var tickets = [];
            commit.replace(/Fixe[sd] #(\d+)/g, function(match, ticket)  {
                  tickets.push(ticket);
               }
            );
            return tickets.length ? commit.replace("TICKETREF", tickets.map(function(ticket)  {
...
;
   function gitLog(format)  {
      var result = exec("git log " + prevVersion + ".." + baseVersion + " " + "--format='" + format + "'",  {
            silent:true         }
      );
      console.log("git log " + prevVersion + ".." + baseVersion + " " + "--format='" + format + "'");
      if (result.code !== 0)  {
