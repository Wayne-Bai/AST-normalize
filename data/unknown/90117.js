...
process.argv.slice(2).forEach(function(url)  {
      oembed.discover(url, function(error, alternates)  {
            ...
            if (jsonUrl && xmlUrl)  {
               ...
               function compare()  {
                  var k;
                  if (! jsonResult || ! xmlResult) return                   for (k in jsonResult)  {
                        if (jsonResult.hasOwnProperty(k) && ! xmlResult.hasOwnProperty(k)) console.warn("Key " + k + " only present in JSON")                         else if (jsonResult[k] !== xmlResult[k]) console.warn(JSON.stringify(jsonResult[k]) + " ≠ " + JSON.stringify(xmlResult[k]))                     }
                  for (k in xmlResult)  {
                        if (! jsonResult.hasOwnProperty(k) && xmlResult.hasOwnProperty(k)) console.warn("Key " + k + " only present in XML")                     }
               }
;
