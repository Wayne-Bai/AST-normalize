...
function runTool(platform, args)  {
   ...
   var traceListener = wtf.analysis.createTraceListener( {
         ...
         wtf.timing#frameStart:function(e)  {
            ...
            log("frame(" + frameNumber + ", " + """ + util.formatTime(e.time) + """ + ", function(ctxs, objs, resources) {");
            log("  var result;");
         ...
         webglcapture.createContext:function(e)  {
            log("  createContext(" + e.args["handle"] + ", " + e.args["attributes"] + ")");
         }, 
         ...
         custom:function(e)  {
            if (e instanceof wtf.analysis.ScopeEvent && e.eventType.name.indexOf("WebGLRenderingContext#") == 0)  {
               var callName = e.eventType.name.substr(22);
               var offsetMs = util.pad("+" + e.time - frameStart | 0 + "ms", - 6);
               var argString = "";
               switch(callName) {
                  case "bufferData":
                        ...
                        argString = [e.args["target"], data, e.args["usage"]].join(", ");
                        break;
                  ...
                  case "texImage2D":
                        ...
                        switch(e.args["dataType"]) {
                           case "pixels":
                                 ...
                                 argString = [e.args["target"], e.args["level"], e.args["internalformat"], e.args["width"], e.args["height"], e.args["border"], e.args["format"], e.args["type"], "new Uint8Array([" + e.args["pixels"] + "])"].join(", ");
                                 break;
                              
                           case "null":
 
                                 argString = [e.args["target"], e.args["level"], e.args["internalformat"], e.args["width"], e.args["height"], e.args["border"], e.args["format"], e.args["type"], "null"].join(", ");
                                 break;
                              
                           default:
 
                                 argString = [e.args["target"], e.args["level"], e.args["internalformat"], e.args["format"], e.args["type"], "resources[" + nextResourceId + "]"].join(", ");
                                 resources.push("{" + "  id: " + nextResourceId + "," + "  type: "image"," + "  mimeType: "" + e.args["dataType"] + ""," + "  data: new Uint8Array([" + e.args["pixels"] + "])" + "}");
                                 nextResourceId++;
                                 break;
                  ...
                  case "texSubImage2D":
                        ...
                        switch(e.args["dataType"]) {
                           case "pixels":
                                 ...
                                 argString = [e.args["target"], e.args["level"], e.args["xoffset"], e.args["yoffset"], e.args["width"], e.args["height"], e.args["format"], e.args["type"], "new Uint8Array([" + e.args["pixels"] + "])"].join(", ");
                                 break;
                              
                           case "null":
 
                                 argString = [e.args["target"], e.args["level"], e.args["xoffset"], e.args["yoffset"], e.args["width"], e.args["height"], e.args["format"], e.args["type"], "null"].join(", ");
                                 break;
                              
                           default:
 
                                 argString = [e.args["target"], e.args["level"], e.args["xoffset"], e.args["yoffset"], e.args["format"], e.args["type"], "resources[" + nextResourceId + "]"].join(", ");
                                 resources.push("{" + "  id: " + nextResourceId + "," + "  type: "image"," + "  mimeType: "" + e.args["dataType"] + ""," + "  data: new Uint8Array([" + e.args["pixels"] + "])" + "}");
                                 nextResourceId++;
                                 break;
...
;
               log("  /* " + offsetMs + " */ result = gl." + callName + "(" + argString + ");");
               switch(callName) {
                  case "createBuffer":
                  ...
                  case "createTexture":
                        ...
                        log("                " + "objs[" + e.args["value"] + "] = result;");
                        log("                " + "assignObject(gl, result);");
                        break;
