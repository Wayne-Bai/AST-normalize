...
node;
"use strict";
var express = require("express"), app = express(), http = require("http"), server = http.createServer(app), io = require("socket.io").listen(server), port = 3333, host = "0.0.0.0";
require("mootools");
io.set("log level", 1);
...
server.listen(port);
function()  {
   var os = require("os"), ifaces = os.networkInterfaces(), dev;
...
();
var Model = require("./src/epitome-model"), Collection = require("./src/epitome-collection");
var User = new Class( {
      Extends:Model, 
      options: {
         defaults: {
