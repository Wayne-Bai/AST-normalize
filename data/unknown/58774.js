! ✖ / env;
node;
var fs = require("fs"), path = require("path"), root = path.resolve(__dirname, ".."), glob = require("glob"), conversion =  {
   cli:1, 
   api:3, 
   files:5, 
   misc:7}
...
glob(root + "/{README.md,doc/*/*.md}", function(er, files)  {
      if (er) throw er      output(files.map(function(f)  {
               var b = path.basename(f);
               if (b === "README.md") return [0, b]               if (b === "index.md") return null               var s = conversion[path.basename(path.dirname(f))];
               return [s, f];
            }
         ).filter(function(f)  {
               return f;
...
function writeLine(sd)  {
   var sxn = sd[0] || 1, doc = sd[1], d = path.basename(doc, ".md");
