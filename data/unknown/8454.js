! ✖ / env;
node;
var commander = require(__dirname + "/commander"), path = require("path"), fork = require("child_process").fork;
var ZERVER = __dirname + "/zerver", WATCHER = __dirname + "/watcher", PACKAGE = __dirname + "/../package.json", ENV_MATCHER = /([^\,]+)\=([^\,]+)/g, API_DIR = "zerver", CHANGE_TIMEOUT = 1000;
startServer();
function processFlags()  {
   ...
   var zerverVersion;
   try {
      var packageFile = require("fs").readFileSync(PACKAGE), packageData = JSON.parse(packageFile);
   ...
   commands.version(zerverVersion, "-v, --version").usage("[options] [dir]").option("-r, --refresh", "auto-refresh browsers on file changes").option("-c, --cli", "js shell for connect remote clients").option("-m, --manifest <paths>", "declare HTML5 appCache manifest files").option("-p, --production", "enable production mode (caching, concat, minfiy, gzip, etc)").option("-P, --port <n>", "set server port to listen on", parseInt, process.env.PORT || 5000).option("-V, --verbose", "verbose request logging").option("-l, --less", "automatically compile less into css").parse(args);
...
function startServer()  {
   ...
   var death = false, cwd = commands.args[0] ? path.join(process.cwd(), commands.args[0]) : process.cwd(), apiCheck = new RegExp("^" + cwd + "/" + API_DIR), args = [new Buffer(JSON.stringify( {
            port:commands.port, 
            apiDir:API_DIR, 
            apiURL:API_DIR, 
...
;
   ...
   var watcher = require(WATCHER), lastChange = null;
