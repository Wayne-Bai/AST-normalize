...
function masterServer(debugMode)  {
   ...
   var List = require("List").List, freeList = new List(), activeList = new List();
...
function lockServer(debugMode)  {
   var gid = process.getgid(), uid = process.getuid(), user = process.getpwnam(Config.user), group = process.getgrnam(Config.group), setuid = process.setuid, setgid = process.setgid, setChildUser = function()  {
   }
;
   if (uid && Config.port < 1024)  {
      console.log(["Config.port is set to " + Config.port, "But the server is being run as " + process.getpwuid(uid).name + ".", "Only programs with root user privileges can bind to ports below 1024."].join("
"));
      process.exit(1);
   }
   if (! uid)  {
...
;
         ...
         if (! user)  {
            console.log(["Config.user set to " + Config.user, "But that user could not be found in the system."].join("
"));
         }
         if (! group)  {
            console.log(["Config.group set to " + Config.group, "But that group could not be found in the system."].join("
"));
         }
         process.exit(1);
      }
