...
node;
const;
express = require("express"), path = require("path"), browserid = require("../lib/browserid");
var exampleServer = express.createServer();
exampleServer.use(express.logger( {
         format:"dev"      }
   )).use(express.static(path.join(__dirname, "."))).use(express.bodyParser());
var localOrigin;
...
exampleServer.listen(process.env["PORT"] || 8080, process.env["HOST"] || "127.0.0.1", function()  {
