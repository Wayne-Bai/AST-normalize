...
function as_toplevel(input)  {
   if (input instanceof U.AST_BlockStatement) input = input.body    else if (input instanceof U.AST_Statement) input = [input]    else throw new Error("Unsupported input syntax")   var toplevel = new U.AST_Toplevel( {
         body:input      }
   );
...
function parse_test(file)  {
   ...
   var tw = new U.TreeWalker(function(node, descend)  {
         if (node instanceof U.AST_LabeledStatement && tw.parent() instanceof U.AST_Toplevel)  {
...
;
      var tw = new U.TreeWalker(function(node, descend)  {
            ...
            if (node instanceof U.AST_LabeledStatement)  {
               assert.ok(node.label.name == "input" || node.label.name == "expect", tmpl("Unsupported label {name} [{line},{col}]",  {
               ...
               if (stat instanceof U.AST_BlockStatement)  {
                  if (stat.body.length == 1) stat = stat.body[0]                   else if (stat.body.length == 0) stat = new U.AST_EmptyStatement()               }
...
function make_code(ast, beautify)  {
   if (arguments.length == 1) beautify = true   var stream = U.OutputStream( {
         beautify:beautify      }
   );
   ast.print(stream);
   return stream.get();
...
function evaluate(code)  {
   if (code instanceof U.AST_Node) code = make_code(code)   return new Function("return(" + code + ")")();
}
;
