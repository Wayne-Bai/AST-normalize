...
fs.readFile("app-settings.json", function(err, data)  {
      ...
      var strings = [];
      var n = doc.node("PreferenceScreen").attr( {
            xmlns:android:"http://schemas.android.com/apk/res/android"         }
      );
      var addSettings = function(parent, config)  {
         if (config.type == "group")  {
            var g = parent.node("PreferenceCategory").attr( {
...
;
            switch(config.type) {
               case "combo":
                     ...
                     var titles = res.node("string-array").attr( {
                           name:config.name                        }
                     ), values = res.node("string-array").attr( {
                     ...
                     parent.node("ListPreference").attr(attr);
                     break;
