! ✖ / bin / node;
var commander = require("commander"), fs = require("fs"), colors = require("colors"), forever = require("forever"), os = require("os"), exec = require("child_process").exec, http = require("http"), querystring = require("querystring");
fs.readFile(__dirname + "/info_goorm.json", "utf8", function(err, contents)  {
      if (err != null)  {
         ...
         commander.command("start [option]").option("-d, --daemon", "run the goorm server as a daemon using the forever module...").option("-p, --port [PORT NUM]", "run the goorm server with port which you want...").option("-h, --home [HOME Directory]", "set HOME directory in server").option("-w, --workspace [WORKSPACE Directory]", "set WORKSPACE directory in server").option("--redis-mode", "run the goorm with redis-server").action(function(env, options)  {
               var process_options = [];
...
;
                  fs.exists(process.env.HOME + "/.goorm/config.json", function(exists)  {
                        ...
                        if (raw_config_data && typeof raw_config_data != "object") config_data = JSON.parse(raw_config_data)                        if (! config_data.users || config_data.users.length == 0)  {
                           if (! config_data.users) config_data.users = []                           var readline = require("readline");
...
;
         ...
         commander.command("restart").action(function(env, options)  {
...
;
         ...
         commander.command("stop").action(function(env, options)  {
               forever.list(null, function(format, list)  {
                     var get_current_project_path = function(raw)  {
                        var path = raw.parent.rawArgs[1].split("/");
                        path.pop();
...
;
         ...
         commander.command("set [option]").option("-w, --workspace [dir_name]", "Set the workspace directory").option("-t, --temp-directory [dir_name]", "Set the temporary directory").option("-u, --user [user_id]", "Set the user").option("-x, --plugin_exclude_list [plugin_exclude_list]", "Set the plugin list you want to exclude plugin loading (ex)["org.goorm.plugin.c","org.goorm.plugin.cpp","org.goorm.plugin.java"]").action(function(env, options)  {
               if (! fs.existsSync(process.env.HOME + "/.goorm/"))  {
                  fs.mkdirSync(process.env.HOME + "/.goorm/");
                  fs.writeFileSync(process.env.HOME + "/.goorm/config.json", "", "utf8");
               }
               ...
               if (fs.existsSync(process.env.HOME + "/.goorm/"))  {
                  var config_data =  {} ;
                  var raw_config_data = fs.readFileSync(process.env.HOME + "/.goorm/config.json", "utf8");
                  if (raw_config_data && typeof raw_config_data != "object") config_data = JSON.parse(fs.readFileSync(process.env.HOME + "/.goorm/config.json", "utf8"))                  var workspace = config_data.workspace || process.env.PWD + "/" + "workspace/";
                  ...
                  if (workspace && workspace[workspace.length - 1] != "/") workspace = workspace + "/"                  if (temp_dir && temp_dir[temp_dir.length - 1] != "/") temp_dir = temp_dir + "/"                  var users = config_data.users || [];
                  if (options["user"])  {
...
;
         ...
         commander.command("clean").action(function(env, options)  {
               if (fs.existsSync(process.env.HOME + "/.goorm/"))  {
