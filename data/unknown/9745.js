...
OPTIMIST.describe("l", "List all available tasks").alias("l", "list").boolean("l").describe("h", "Display usage").alias("h", "help").boolean("h").describe("version", "Build version number (x.x.x) defaults 'NEXT'").string("version").default("version", "NEXT").describe("tasks", "Task to run options: [ALL, BUILDDOCS, BUILDSOURCE, CLEAN]").default("tasks", "ALL").describe("format", "Formatting minified JS :[STANDARD, PRETTY_PRINT]").string("format").default("format", "STANDARD").usage("Build Task Manager for " + PROJECT_NAME + "
Usage
...
Build Source Task Complete", "green"));
            if (shouldBuildDocs)  {
               buildDocsTask(version, function(success)  {
...
function buildDocsTask(version, completeHandler)  {
   ...
   CHILD_PROCESS.exec(yuidocCommand.join(" "), function(error, stdout, stderr)  {
         ...
         CHILD_PROCESS.exec(zipCommand, function(error, stdout, stderr)  {
               if (error !== null)  {
