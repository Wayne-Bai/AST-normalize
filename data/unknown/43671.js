...
var nconf = require("nconf").argv().env().add("local",  {
      type:"file", 
      file:"etc/local.json"   }
).add("default",  {
      type:"file", 
      file:"etc/default.json"   }
);
...
assert(node_name != null && node_name.toString().length > 0, "ERROR: missing node-name for replicake node.");
