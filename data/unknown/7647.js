...
run("node_modules/browserify/bin/cmd.js .temp.js", function(error, stdout)  {
      if (error) throw error      var header = ["/*", " * Support for source maps in V8 stack traces", " * https://github.com/evanw/node-source-map-support", " */"].join("
");
...
");
      ...
      run("curl -d @.temp.js "http://closure-compiler.appspot.com/compile"", function(error, stdout)  {
            if (error) throw error            var code = header + "
" + stdout;
...
run("node_modules/coffee-script/bin/coffee --map --compile browserify-test/script.coffee", function(error)  {
      if (error) throw error      run("node_modules/browserify/bin/cmd.js --debug browserify-test/script.js > browserify-test/compiled.js", function(error)  {
            if (error) throw error         }
      );
   }
);
...
run("node_modules/coffee-script/bin/coffee --map --compile header-test/script.coffee", function(error)  {
      if (error) throw error      var contents = fs.readFileSync("header-test/script.js", "utf8");
