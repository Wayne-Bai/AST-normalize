...
program.option("-u, --url [url]", "URL to website that should be proofread.").option("-f, --file [path]", "Path to HTML or Markdown file that should be proofread.").option("-l, --file-list [path]", "Path to a list of files that should be proofread.").option("-c, --config-file [path]", "Path to a custom configuration file.").parse(process.argv);
...
config.dictionaries["build-in"].forEach(function(dictName)  {
      proofreader.addDictionary(path.join(__dirname, "../dictionaries/" + dictName + ".dic"), path.join(__dirname, "../dictionaries/" + dictName + ".aff"));
   }
);
...
sourceLoader.load().then(function(sources)  {
      return Promise.all(sources.map(function(source)  {
               ...
               return proofreader.proofread(toHTML(source.path, source.content)).then(function(result)  {
