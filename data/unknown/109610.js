...
options.version(package.version).option("-i, --input [file]", "Input (csv file) [stdin]", "-").option("-o, --output [file]", "Output (csv file) [stdout]", "-").option("-s, --street-col [col]", "Street col [street]", "street").option("-z, --zipcode-col [col]", "Zipcode col [zipcode]", "zipcode").option("-c, --city-col [col]", "City col [city]", "city").option("-S, --state-col [col]", "State col [state]", "state").option("--zipcode-filter [zips]", "Only geocode records in certain zipcodes, comma separated", "").option("--state-filter [states]", "Only geocode records in certain states, comma separated", "").option("-d, --delimiter [symbol]", "CSV delimiter in input file", ",").option("-O, --output-split [column]", "Write to multiple files divided by column", "").option("--truncate-split [length]", "Used with --output-split, truncate column to first X characters", 0).option("-a, --auth-id [id]", "SmartyStreets auth id [environment variable smartystreets_auth_id]", process.env.SMARTYSTREETS_AUTH_ID).option("-A, --auth-token [token]", "SmartyStreets auth token [environment variable smartystreets_auth_token]", process.env.SMARTYSTREETS_AUTH_TOKEN).option("-j, --concurrency [jobs]", "Maximum number of concurrent requests [48]", 48).option("-✈, --column-definition [mode]", "Column definition mode or file [standard]", "standard").option("-p, --column-prefix [text]", "Prefix for smartystreets columns in the output file [ss_]", "ss_").option("-x, --column-suffix [text]", "Suffix for smartystreets columns in the output file", "").option("-q, --quiet", "Quiet mode - turn off progress messages").option("-l, --log-interval [num]", "Show progress after every X number of rows [1000]", 1000).parse(process.argv);
...
readStream.pipe(csv( {
   ...
   )).pipe(grouper(70)).pipe(geocoder(options)).pipe(writeStream);
