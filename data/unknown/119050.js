...
function(console, args, readFileFunc)  {
   var fileName, env, fs, vm, path, exec, rhinoContext, dir, nodeRequire, nodeDefine, exists, reqMain, loadedOptimizedLib, version = "1.0.2", jsSuffixRegExp = /\.js$/, commandOption = "", rhinoArgs = args, readFile = typeof readFileFunc !== "undefined" ? readFileFunc : null;
   function showHelp()  {
...
;
   ...
   function()  {
      var version = "1.0.2", commentRegExp = /(\/\*([\s\S]*?)\*\/|([^:]|^)\/\/(.*)$)/gm, cjsRequireRegExp = /require\(\s*["']([^'"\s]+)["']\s*\)/g, currDirRegExp = /^\.\//, jsSuffixRegExp = /\.js$/, ostring = Object.prototype.toString, ap = Array.prototype, aps = ap.slice, apsp = ap.splice, isBrowser = ! ! typeof window !== "undefined" && navigator && document, isWebWorker = ! isBrowser && typeof importScripts !== "undefined", readyRegExp = isBrowser && navigator.platform === "PLAYSTATION 3" ? /^complete$/ : /^(complete|loaded)$/, defContextName = "_", isOpera = typeof opera !== "undefined" && opera.toString() === "[object Opera]", empty =  {} , contexts =  {} , globalDefQueue = [], interactiveScript = null, checkLoadedDepth = 0, useInteractive = false, req, cfg =  {} , currentlyAddingScript, s, head, baseElement, scripts, script, src, subPath, mainScript, dataMain, i, ctx, jQueryCheck, checkLoadedTimeoutId;
...
;
         function execManager(manager)  {
            var i, ret, err, errFile, errModuleTree, cb = manager.callback, map = manager.map, fullName = map.fullName, args = manager.deps, listeners = manager.listeners, cjsModule;
            if (cb && isFunction(cb))  {
...
;
         function getManager(map, shouldQueue)  {
            var fullName = map.fullName, prefix = map.prefix, plugin = prefix ? plugins[prefix] || plugins[prefix] = defined[prefix] : null, manager, created, pluginManager;
            if (fullName)  {
         ...
         function prepare_ifs(ast)  {
            ...
            function redo_if(statements)  {
               ...
               for (var i = 0; i < statements.length; ++i)  {
                     ...
                     var ret = statements.slice(0, i).concat([[fi[0], conditional, t, e]]);
                     return redo_if(ret);
                  }
               return statements;
            }
...
;
            ...
            function _lambda(name, args, body)  {
               ...
               body = with_scope(body.scope, function()  {
                     ...
                     if (! is_defun && name && ! scope.references(name)) name = null                     return ret;
                  }
               );
               return [this[0], name, args, body];
            }
...
;
         parse.findDependencies = function(fileName, fileContents)  {
            var dependencies = parse.getAnonDeps(fileName, fileContents), astRoot = parser.parse(fileContents);
            parse.recurse(astRoot, function(callName, config, name, deps)  {
                  if (name && isArrayLiteral(name))  {
                     deps = name;
                     name = null;
         ...
         var pragma =  {
            ...
            namespace:function(fileContents, ns, onLifecycleName)  {
               if (ns)  {
                  ...
                  fileContents = fileContents.replace(pragma.defineJQueryRegExp, "typeof " + ns + ".define === 'function' && " + ns + ".define.amd && " + ns + ".define.jQuery");
...
;
      optimize =  {
         ...
         jsFile:function(fileName, outFileName, config, moduleName, pluginCollector)  {
            var parts = config.optimize + "".split("."), optimizerName = parts[0], keepLines = parts[1] === "keepLines", licenseContents = "", fileContents, optFunc, match, comment;
...
;
         require.load = function(context, moduleName, url)  {
            ...
            if (require._isSupportedBuildUrl(url))  {
               ...
               try {
                  if (url in cachedFileContents && ! context.needFullExec[moduleName] || context.fullExec[moduleName])  {
...
;
      ...
      build.toTransport = function(anonDefRegExp, namespace, moduleName, path, contents, layer)  {
         return contents.replace(anonDefRegExp, function(match, start, callName, possibleComment, suffix, namedModule, namedFuncStart)  {
               ...
               return start + namespace + "define('" + moduleName + "'," + deps ? "[" + deps.toString() + "]," : "" + namedModule ? namedFuncStart.replace(build.leadingCommaRegExp, "") : suffix;
            }
         );
      }
;
