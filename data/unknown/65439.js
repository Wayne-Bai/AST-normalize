...
var cwd = process.cwd();
if (args.mode === "browser") args.visible = truevar ten_years = 315360000;
...
function writeResponse(response, status, data, type, headers)  {
   if (verbose) console.log("response message:", data)   type = type || "text/plain";
   headers = headers ||  {} ;
   headers["Content-Type"] = type;
   response.writeHead(status, headers);
   if (data)  {
      if (type !== "text/plain") response.write(data, "binary")       else response.write(data)   }
   response.end();
}
;
...
;
   ...
   request.on("end", function()  {
         ...
         var decoded;
         if (request.is("application/x-www-form-urlencoded")) decoded = querystring.parse(body)          else if (request.is("json")) decoded = JSON.parse(body)          else throw new Error("cannot handle content-type: " + request.get("Content-Type"))         if (options.dump)  {
...
***
", function()  {
                  if (verbose) console.log("decoded body", decoded)                  writable.write(JSON.stringify(decoded));
                  writable.end();
               }
            );
         }
            ...
            switch(decoded.command) {
               ...
               case "autosave":
                     ...
                     if (! no_response_on_save)  {
                        if (too_old_on_save) messages.push( {
                              type:"version_too_old_error"                           }
                        )                        if (precondition_fail_on_save) status = 412                         else if (fail_on_save) status = 400                         else success()                     }
                     break;
                  
               case "recover":
 
                     if (! no_response_on_recover)  {
                        if (! fail_on_recover) success()                         else status = 400                     }
                     break;
...
function runserver()  {
   if (! ip)  {
    ...
    else app.listen(port, ip)   var driver, xvfb, wm;
   var failures = [];
...
");
                  for (var i = 0, failure; failure = failures[i]; ++i)  {
                        ...
                        if (error.err.stack)  {
                           ...
                           var err = error.err;
                           err = typeof err !== "string" ? JSON.stringify(err, undefined, 4) : err.replace(/^/gm, "    ");
                  ...
                  console.log("Elapsed: " + d.getHours() + "h" + d.getMinutes() + "m" + d.getSeconds() + "s");
                  if (stats.failures > 0) console.log(colors.red("Failures: " + stats.failures))                  console.log("Total: " + stats.tests);
                  if (args.mode === "runner")  {
                     driver.quit().then(function()  {
                           if (xvfb) xvfb.kill()                           process.exit(result.failures ? 1 : 0);
                        }
                     );
                  }
                  break;
...
;
   ...
   function rundriver(nohtml)  {
      ...
      var query =  {} ;
      if (nohtml) query.nohtml = "1"      if (args.grep) query.grep = args.grep      var location = url.format( {
            protocol:"http", 
            hostname:ip, 
            port:port, 
...
;
   if (args.mode === "server") return    if (! args.visible)  {
      headless( {
            display: {
               width:1680, 
               height:1050            }} , function(err, xvfb_, server_no)  {
            xvfb = xvfb_;
            process.env.DISPLAY = ":" + server_no;
            wm = child_process.spawn("fvwm2",  {
                  stdio:"ignore"               }
            );
            rundriver(true);
         }
