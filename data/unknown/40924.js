...
if (opts.get("man") && ! opts.get("build"))  {
   ...
   var man = spawn("man", ["--warnings", "-E UTF-8", "-l", "-"],  {
         LANG:"C"      }
   );
   ...
   var fHtml = null;
   var fFrag = null;
   if (! opts.get("html") && ! opts.get("fragment")) fRoff = ronn.roff()    else  {
      if (opts.get("roff")) fRoff = ronn.roff()      if (opts.get("html")) fHtml = ronn.html()      if (opts.get("fragment"))  {
         if (opts.get("html"))  {
   ...
   if (opts.get("build"))  {
      if (fRoff) fs.writeFileSync(fBase + ".roff", fRoff, "utf8")      if (fHtml) fs.writeFileSync(fBase + ".html", fHtml, "utf8")      if (fFrag) fs.writeFileSync(fBase + ".fragment", fFrag, "utf8")   }
    else  {
      if (fRoff) sys.puts(fRoff)      if (fHtml) sys.puts(fHtml)      if (fFrag) sys.puts(fFrag)   }
}
