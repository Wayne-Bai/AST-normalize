...
function color(val, text)  {
   return tty ? "[" + val + "m" + text + "[39m" : text;
}
;
function bold(text)  {
   return tty ? "[1m" + text + "[22m" : text;
}
;
var fs = require("fs");
...
zlib.inflate(data, function(err, data)  {
      if (err) throw err      var tile = new VectorTile(new Protobuf(new Uint8Array(data)));
            ...
            for (var i = 0; i < layer.length; i++)  {
                  ...
                  for (var key in feature)  {
                        if (feature.hasOwnProperty(key) && key[0] !== "_")  {
                           if (! key in tags) tags[key] = []                           tags[key].push(feature[key]);
                        }
...
;
            ...
            for (var key in tags)  {
                  ...
                  for (var i = 0; i < tags[key].length; i++)  {
                        var val = tags[key][i];
                        if (counts[val]) counts[val].count++                         else counts[val] =  {
                           val:val, 
                           count:1                        }
                     }
                  counts = Object.keys(counts).map(function(k)  {
                        return counts[k];
                     }
                  );
