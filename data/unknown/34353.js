...
function AppLoader(appUrl, initMsg)  {
   var urls = ["", "shared/jquery.js", "shared/shiny.js", "shared/shiny.css", "shared/slider/css/jquery.slider.min.css", "shared/slider/js/jquery.slider.min.js", "shared/bootstrap/css/bootstrap.min.css", "shared/bootstrap/js/bootstrap.min.js", "shared/bootstrap/css/bootstrap-responsive.min.css", "__assets__/sockjs-0.3.min.js", "shared/bootstrap/img/glyphicons-halflings.png", "shared/slider/img/jslider.plastic.png", "__sockjs__/info"];
...
function()  {
   this.run_p = function()  {
      return Q.all([this._urlLoader.run_p(), this._wsLoader.run_p()]);
   }
;
}
...
function()  {
   this.run_p = function()  {
      ...
      var options =  {} ;
      if (typeof SOCKET_PATH === "string") options.socketPath = SOCKET_PATH      var ws = new WebSocket.Client(this._url, undefined, options);
      function onOpen()  {
         var initMsg = JSON.stringify(self._initMsg);
         if (SHINY_SERVER) initMsg = JSON.stringify([initMsg])         ws.send(initMsg);
      }
;
      ws.on("open", function(event)  {
            if (! SHINY_SERVER) onOpen()         }
      );
      ws.on("message", function(event)  {
            try {
               var messages;
               if (event.data === "o")  {
                  if (SHINY_SERVER) onOpen()                  return ;
               }
...
function UrlLoader(baseUrl, urls, filter)  {
   this._results =  {} ;
   this._urls = urls.map(function(url)  {
         return baseUrl + url;
      }
   );
   this._filter = filter;
...
");
                              deferred.reject(new Error("Unexpected results for URL " + url));
                              return ;
                           }
                        }
...
var promises = [];
for (var i = 0; i < sessionCount; i++) promises.push(appLoader.run_p())Q.all(promises).done();
