...
).use("json", "parallel", "gallery-funcprog", "gallery-log-filter");
function ✖(Y)  {
   "use strict";
   var mod_fs = require("fs"), mod_path = require("path"), mod_url = require("url"), mod_qs = require("querystring"), mod_http = require("http"), mod_express = require("express"), mod_request = require("request"), content_type = require("./server/content-type.js");
   var argv = require("optimist").usage("usage: $0").option("config");
    {
      demand:
true, describe;
   ...
   ✖.option("debug");
    {
      boolean:
true, describe;
      "Turn on debugging (causes leaks)";
   }
;
   ✖.option("test");
    {
      describe:
"Name of file to create when initialization is finished";
   }
;
   ✖.argv;
...
m, i;
                     i, f;
                     mod_path.resolve(config.root || "", f);
                  }
;
                  ;
               }
      ...
      var tasks = new Y.Parallel(), results =  {} , code = 200;
      Y.each(module.rejects, function(m)  {
            var relay_url = config.combo + m;
            Y.log("relay: " + relay_url, "debug", "combo-dev");
            mod_request(relay_url,  {
                  pool:false               }, 
               tasks.add(function(error, response, body)  {
