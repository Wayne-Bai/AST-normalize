...
function l(msg, indent, nlBefore, nlAfter)  {
   if (! indent) indent = 0   if (Array.isArray(msg)) msg = msg.join("
")   while (indent--) msg = " " + msg   if (nlBefore) msg = "
" + msg   if (nlAfter) msg = msg + "
...
if (! app.config || ! app.out)  {
   l(["", "  Error: missing required parameters (--config, --out)".error, ""]);
   app.help();
}
