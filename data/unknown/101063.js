...
function main()  {
   ...
   getDefaultMusicDir(function(err, defaultMusicDir)  {
         if (err) throw err         defaultConfig.musicDirectory = defaultMusicDir;
         loadConfig(pathToConfig, function(err, config)  {
               if (err) throw err               if (browserExe || printUrl)  {
               ...
               db.open(function(err)  {
                     if (err)  {
                        if (exitGracefullyIfRunning && /^IO error: lock.*: Resource temporarily unavailable$/.test(err.message))  {
                           return ;
                        }
                         else  {
                           throw err;
...
;
                           createGzipStatic(options, function(err, middleware)  {
                                 if (err) return cb(err)                                 app.use(middleware);
                                 cb();
                              }
                           );
                        }
                     ...
                     pend.wait(function(err)  {
                           if (err) throw err                           var player = new Player(db, config.musicDirectory, config.encodeQueueDuration, config.googleApiKey);
                           player.initialize(function(err)  {
                                 if (err) throw err                                 log.debug("Player initialization complete.");
...
;
         function printMsgAndExit(err)  {
            if (err) throw err            if (brandNewConfig)  {
...
" + missingFields.join(", "));
            ...
            console.error("Take a peek and make sure the values are to your liking,
...
function initializeDownload(app, player, config, hasPermRead)  {
   ...
   app.get("/download/keys", hasPermRead, function(req, resp)  {
         ...
         reqKeys.forEach(function(key)  {
               ...
               if (! dbFile) return                files.push(path.join(musicDir, dbFile.file));
               if (commonAlbumName === undefined) commonAlbumName = dbFile.albumName || ""                else if (commonAlbumName !== dbFile.albumName) commonAlbumName = null               if (commonArtistName === undefined) commonArtistName = dbFile.artistName || ""                else if (commonArtistName !== dbFile.artistName) commonArtistName = null            }
         );
...
;
   function sendZipOfFiles(zipName, files, req, resp)  {
      ...
      resp.setHeader("Content-Disposition", contentDisposition(zipName,  {
               type:"attachment"            }
         ));
      ...
      files.forEach(function(file)  {
            zipfile.addFile(file, path.relative(config.musicDirectory, file),  {
                  compress:false               }
            );
         }
      );
...
function redirectHttpToHttps(req, resp, next)  {
   if (req.socket.encrypted) return next()   resp.writeHead(301,  {
...
function startServer(app, db, player, config, sslEnabled)  {
   ...
   playerServer.init(function(err)  {
         if (err) throw err         createHttpServer(attachWebSocketServer);
      }
   );
   function createHttpServer(cb)  {
      if (! sslEnabled)  {
         ...
         if (config.sslKey === defaultConfig.sslKey || config.sslCert === defaultConfig.sslCert)  {
...
");
      ...
      function readKeyAndCert(cb)  {
         ...
         pend.go(function(cb)  {
               if (! config.sslCaDir) return cb()               options.ca = [];
...
;
         ...
         protocolServer.on("error", function(err)  {
               if (err.code === "EADDRINUSE")  {
                  log.error("Failed to bind MPD protocol to port " + mpdPort + ": Address in use.");
               }
                else  {
                  throw err;
               }
         ...
         protocolServer.listen(mpdPort, mpdHost, function()  {
               log.info("MPD/GrooveBasin Protocol listening at " + mpdHost + ":" + mpdPort);
            }
         );
      }
   }
...
function printUsageAndExit()  {
   process.stderr.write("Usage: groovebasin [options]
" + "
" + "Available Options:
