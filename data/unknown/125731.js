! ✖ / env;
node;
var fs = require("fs"), path = require("path"), marked = require("marked"), dir = __dirname + "/tests";
...
;
var main = function()  {
   if (! files) load()   var complete = 0, keys = Object.keys(files), i_ = 0, l_ = keys.length, filename, file, text, html;
   main:
for (; i_ < l_; i_++)  {
            filename = keys[i_];
            file = files[filename];
            ...
            for (; i < l; i++)  {
                  if (text[i] !== html[i])  {
                     ...
                     html = html.substring(Math.max(i - 30, 0), Math.min(i + 30, html.length));
                     console.log("
...
main.bench = function(name, func)  {
   if (! files)  {
      ...
      fs.readdirSync(__dirname + "/new").forEach(function(name)  {
            if (name.split(".").pop() === "html") return             if (name === "main.text") return             delete files[name];
         }
      );
...
", "");
   ...
   var start = Date.now(), times = 1000, keys = Object.keys(files), i = 0, l = keys.length, filename, file;
   while (times--)  {
         for (i = 0; i < l; i++)  {
               filename = keys[i];
               file = files[filename];
