...
runner.run(module,  {
      ...
      load myfavoritebeer and wait for the signin button to be visible:function(done)  {
         browser.chain( {
               onError:done            }
         ).get(persona_urls["myfavoritebeer"]).wclick(CSS["myfavoritebeer.org"].signinButton, done);
      }, 
      ...
      verify signed in to myfavoritebeer:function(done)  {
         browser.chain( {
               onError:done            }
         ).wwin().wtext(CSS["myfavoritebeer.org"].currentlyLoggedInEmail, function(err, text)  {
               done(err || assert.equal(text, testUser.email));
            }
         );
      }} );
