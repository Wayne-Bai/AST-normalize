...
/*
 * Womack client code below here
 *
 * Based on juggernaut's application.js
 * Copyright (c) 2010 Alexander MacCaw
 *
 */
var Womack = function (options) {
    this.options = options || {};

    var loc = this.locate();
    this.options.host = this.options.host || loc.host || window.location.hostname;
    this.options.port = this.options.port || loc.port || window.location.port;
...
Womack.prototype.proxy = function(func){
  var thisObject = this;
...
Womack.prototype.on = function(name, callback){
  if ( !name || !callback ) return;
  if ( !this.handlers[name] ) this.handlers[name] = [];
  this.handlers[name].push(callback);
};

Womack.prototype.subscribe = function(channel, callback) {
    this.io.emit('subscribe', {'channel': channel})
    this.io.on(channel, function(msg) { callback(msg.data, msg) });
}

// private

Womack.prototype.trigger = function(){
    var args = [];
...
Womack.prototype.onconnect = function(){
    this.sessionID = this.io.socket.sessionid;
    this.trigger("connect");
}

Womack.prototype.ondisconnect = function(){
    this.trigger("disconnect");
}

Womack.prototype.locate = function () {
    ...
    for (var i = 0, ln = scripts.length; i<ln; i++) {
        ...
        if (me = script.match(/^[^:]+:\/\/(.*)\/womack.js$/)) {
            if (hp = me[1].match(/^([^:]+)(?::(\d+))?$/)) {
                options.host = hp[1];
                if (hp[2]) {
                    options.port = parseInt(hp[2], 10);
                }
            }
            return options;
        }
