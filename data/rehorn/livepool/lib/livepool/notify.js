...
exports.request = function(id, req, res) {
    ...
    if (socket && checkUrl(req.url)) {
        ...
        util.exists(path.dirname(headerPath)) && fs.writeFile(headerPath, JSON.stringify(req.headers, null, 4), 'binary', function(err) {
            if (err) throw err;
        });
    }
};
...
exports.response = function(id, req, res, body) {
    ...
    if (socket && checkUrl(req.url)) {
        ...
        util.exists(path.dirname(headerPath)) && fs.writeFile(headerPath, res._header, 'binary', function(err) {
            if (err) throw err;
        });

        var bodyPath = path.join(config.global.tempDir, socket.id, id + '_b.txt');
        util.exists(path.dirname(bodyPath)) && fs.writeFile(bodyPath, body, 'binary', function(err) {
            if (err) throw err;
        });
        ...
        if (config.settings.keep) {
            ...
            if (ctype.indexOf('javascript') >= 0 || ctype.indexOf('css') >= 0) {
                ...
                var args = [
                    path.resolve(__dirname, '../../tools/beautify/scripts/run.js'),
                    bodyFilepath,
                    ctypepath
                ];
                exec(cmd + args.join(' '), {
