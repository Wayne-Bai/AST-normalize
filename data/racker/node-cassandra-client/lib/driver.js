...
// how long we leave a known bad connection in a bad state.
var DEFAULT_STALE_THRESHOLD = 10000;
...
// The default max number of times to retry a query
var DEFAULT_MAX_QUERY_RETRIES = 3;
...
Connection.prototype.connect = function(callback) {
  ...
  this.con = thrift.createConnection(self.connectionInfo.host, self.connectionInfo.port, {'transport' : thrift.TFramedTransport});
  ...
  this.con.on('connect', function() {
    ...
    var login = function(cb) {
      if (self.connectionInfo.user || self.connectionInfo.pass) {
        var creds = new ttypes.AuthenticationRequest({credentials:{username: self.connectionInfo.user, password: self.connectionInfo.pass}});
...
Connection.prototype.execute = function(query, args, callback) {
  ...
  var timeoutId = setTimeout(function() {
    callback(new errors.ClientTimeoutError('Query timed out'), null, metadata);
    timeoutId = null;
  }, self.query_timeout);
...
var ConnectionInPool = module.exports.ConnectionInPool = function(options) {
  options.staleThreshold = options.staleThreshold || DEFAULT_STALE_THRESHOLD;
...
/**
 * An object representing a query being executed by a PooledConnection.
 * @param {String} query The query to be executed.
 * @param {Array} args The args to be used with the query.
 * @constructor
 */
var PooledConnectionQuery = function(query, args) {
  this.query = query;
  this.args = args;
  this.attempts = 0;
  this.results = null;
 ...
 */
PooledConnectionQuery.prototype.shouldRetry = function(max_query_retries) {
  return !this.success && this.attempts < max_query_retries;
};
...
var PooledConnection = module.exports.PooledConnection = function(config) {
  ...
  this.max_query_retries = config.max_query_retries || DEFAULT_MAX_QUERY_RETRIES;
...
PooledConnection.prototype.execute = function(query, args, executeCallback) {
  var self = this,
      pooledQuery = new PooledConnectionQuery(query, args);
  ...
  async.whilst(
    pooledQuery.shouldRetry.bind(pooledQuery, self.max_query_retries),
    ...
    function executeQuery(callback) { 
      self._getNextCon(function(err, con) {
        ...
        try {
          con.execute(pooledQuery.query, pooledQuery.args, function(err, result, metadata) {
            if (err) {
              if ((err instanceof BindError) || (err.hasOwnProperty('name') && contains(appExceptions, err.name))) {
                pooledQuery.metadata = metadata;
                callback(err);
              } else {
            ...
            pooledQuery.result = result;
            pooledQuery.metadata = metadata;
            pooledQuery.success = true;
            callback();
          });
        } catch (e) {
    ...
    function finished(err) {
      ...
      if (err) {
        self.emit('log', 'error', 'Unable to perform query due to error', {err: err, query: pooledQuery.query, args: pooledQuery.args});
        callback(err);
        return;
      } else if (!err && !pooledQuery.success && pooledQuery.attempts === self.max_query_retries) {
        errMsg = 'Giving up performing query after ' + self.max_query_retries + ' retries';
        self.emit('log', 'error', errMsg);
        callback(new errors.ClientTimeoutError(errMsg));
        return;
      }
      callback(null, pooledQuery.result, pooledQuery.metadata);
    }
  );
};
