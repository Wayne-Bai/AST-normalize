/*global module, expect, stop, start, strictEqual, ok, test, equal */
/*jshint strict: false */
module( "callbacks" );
...
(function() {
	...
	var each = function( obj, callback, args ) {
		...
		if ( args ) {
			throw Error("Didn't expect that!");
	...
	each( tests, function( strFlags, resultString ) {

		var objectFlags = {};

		each( strFlags.split( " " ), function() {
			if ( this.length ) {
				objectFlags[ this ] = true;
			}
		});

		each( filters, function( filterLabel ) {
...
test( "jQuery.Callbacks.fireWith - arguments are copied", function() {
	...
	var cb = window.Callbacks("memory"),
		args = ["hello"];
...
test( "jQuery.Callbacks() - adding a string doesn't cause a stack overflow", function() {
	...
	window.Callbacks().add( "hello world" );
